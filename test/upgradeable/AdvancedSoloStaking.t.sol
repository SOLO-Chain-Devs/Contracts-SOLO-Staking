...
contract SOLOStakingFailingTest is Test {
    ...
    function logEth(string memory label, uint256 value) internal pure {
        ...
        console.log(string.concat("%s", padding, "%d.%d"), label, value / 1 ether, value % 1 ether);
    }

    function logBool(string memory label, bool value) internal pure {
        string memory padding = _getPadding(label);
        console.log(string.concat("%s", padding, "%s"), label, value ? "true" : "false");
    }
    ...
    function test_YieldCalculationSimpleMultiUser() public {
        ...
        for (uint256 week = 1; week <= 4; week++) {
            ...
            for (uint256 i = 0; i < users.length; i++) {
                ...
                uint256 periodRebaseAmount =
                    (stSOLOToken.tokensPerYear() * timeElapsed) / stSOLOToken.SECONDS_PER_YEAR();
                uint256 shareIncrement =
                    (periodRebaseAmount * stSOLOToken.PRECISION_FACTOR() * 2) / stSOLOToken.getTotalNormalShares();
    ...
    function test_RequestWithdrawal() public {
        ...
        (uint256[] memory soloAmounts,,, bool[] memory processed) = stakingContract.getPendingWithdrawals(alice);
    ...
    function test_WithdrawalWithDebug() public {
        ...
        // Final assertions
        assertEq(soloToken.balanceOf(alice), INITIAL_AMOUNT, "SOLO balance should return to initial amount");
        assertEq(stSOLOToken.balanceOf(alice), 0, "stSOLO balance should be zero after withdrawal");
    }
